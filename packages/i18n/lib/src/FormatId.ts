 ...
 *
 * Multiple calls to declareFormat with the same id will yield identical `FormatId`s.
	...
	if (!cached) {
		...
		if (process.env.NODE_ENV !== 'production') {
			if (cached.defaultFormat !== data.defaultFormat) {
				console.error(
					`Default format must be static. Was "${
						cached.defaultFormat
					}", is now "${data.defaultFormat}".`
				);
			}
		}
	}
